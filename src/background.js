...
chrome.runtime.onInstalled.addListener(function(details){
    if(details.reason == "install"){
        console.log("first installation; saving dict to storage");
        chrome.storage.local.set({'the_categories': orig_categorized_labels}, function() {
          console.log('dict  saved');
        });
    }else if(details.reason == "update"){
	console.log("update; not overriding dict; might contain personal details");
    }
});  


//some on click heandler will update storage with user settings here...
...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    ...
    if (request.what == "get_dict") {
	sendResponse({status: "got the message. working on it... will send response when done"});
	chrome.storage.local.get("the_categories", function (obj) { 
	  var categorized_labels = obj.the_categories;
	  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
	    chrome.tabs.sendMessage(tabs[0].id, { status: "categories and colors", 
            the_categories: categorized_labels,the_colors: colors}, 
            null)
	  });
	});
    }
    else {
        sendResponse({status: "don't know what to do with that..."});
    }
  }
);
